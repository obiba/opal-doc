...
``org.obiba.magma.r.copyBufferMemoryRatio``              The ratio of memory (memory used by the buffer divided by the available memory) to determine the size of the table values buffer when writing data to R by batch. Default value is ``0.5``. 
